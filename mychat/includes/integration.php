<?php
   ...
   function get_user_id() 
	...
	function get_friend_list($userid, $time)
	{
		...
		$sql = ("
			...
			WHERE " . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_USERID . " = '" . $db->escape_string($userid) . "' 
				AND " . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_FRIENDS . " = 2 
	...
	function get_online_list($userid, $time) 
	{
		...
		/*$sql = ("
			...
			WHERE ('" . time() . "' - arrowchat_status.session_time - 60 < '" . $online_timeout . "') 
				AND " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " != '" . $db->$userid . "' 
			ORDER BY " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " ASC
		");
		$sql = ("
		...
		$sql = ("
            ...
            WHERE " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " != '" . $db->escape_string($userid) . "'
                AND (kidskula_users.id IN(SELECT kidskula_friend_request.receiver_id FROM kidskula_friend_request WHERE kidskula_friend_request.sender_id='".$db->escape_string($userid)."' AND kidskula_friend_request.status='2') 
				OR kidskula_users.id IN(SELECT kidskula_friend_request.sender_id FROM kidskula_friend_request WHERE kidskula_friend_request.receiver_id='".$db->escape_string($userid)."' AND kidskula_friend_request.status='2'))
            ORDER BY " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " ASC
        ");
		return $sql; 
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if ($image!='') 
		{
			return '/~kidskula/web/upload/profilepicture/chatimages/' . $image;
		} 
		else 
		{
