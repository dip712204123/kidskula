	...
	*/
	...
	if (logged_in($userid)) 
	{
		...
		else 
		{
			...
			$db->execute("
				INSERT INTO arrowchat_status (userid, chatroom_window, chatroom_stay, hash_id, session_time,'status') 
				VALUES ('" . $db->escape_string($userid) . "', '-1', '-1', '" . $hash_id . "', '" . time() . "','available')
			");
		}
