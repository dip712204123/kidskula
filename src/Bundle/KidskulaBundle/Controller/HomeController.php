...
//        return $this->render('BundleKidskulaBundle:Home:home.html.twig',array('username'=>$username));
    ...
    public function messagedetailsAction($messageid,$replyid) {
        ...
        $em = $this->getDoctrine()->getManager();
        $check_for = $em->getRepository('BundleAdminBundle:KidskullaMessaging')->findOneBy(array('id' => $messageid,'status' => '1'));
        $check_for->setSeenbyuser('1');
        $em->flush();
        
        $em = $this->getDoctrine()->getManager();
        $check_for = $em->getRepository('BundleAdminBundle:KidskullaMessaging')->findOneBy(array('id' => $replyid,'status' => '1'));
        $check_for->setSeenbyuser('1');
        $em->flush();
        
        $message = $em->getRepository('BundleAdminBundle:KidskullaMessaging')->findOneBy(array('id' => $messageid,'status' => '1'));
        
        $replymessage = $em->getRepository('BundleAdminBundle:KidskullaMessaging')
                ->findBy(array('fromuser' => $message->getTouser(),'parentid' => $messageid,'status' => '1'),array('id'=>'desc'));
                
                
                 
                
        //echo '<pre>';
        //\Doctrine\Common\Util\Debug::dump($replymessage);
        //exit;
         if($request->getMethod()=='POST'){
             
             $messagetext = $request->get('answere');  
                
                $entities = new KidskullaMessaging();
                
                if($this->getUser() == $message->getFromuser() )
                {
                ...
                $entities->setTouser($touser);
                $entities->setFromuser($this->getUser());
                $entities->setParentid($messageid);
                $entities->setTexttoshow($messagetext);
        ...
        return $this->render('BundleKidskulaBundle:Message:messagedetails.html.twig', array(
    ...
    public function replymessageAction($messageid){
        ...
        $receiverId = $this->getUser();
        $replymessage = $em->getRepository('BundleAdminBundle:KidskullaMessaging')->findBy(array('parentid' => $messageid,'status' => '1'),array('id'=>'desc'));
        //echo '<pre>';
        //\Doctrine\Common\Util\Debug::dump($replymessage);
        //exit;
        return $this->render('BundleKidskulaBundle:Message:replymessage.html.twig', array(
    ...
    {
        return $this->render('BundleKidskulaBundle:Message:messagelist.html.twig');
    }
