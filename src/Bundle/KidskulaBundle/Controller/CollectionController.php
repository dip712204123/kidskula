...
class CollectionController extends Controller {
    ...
    function mycollection_trade_accpetAction(Request $request) {
	   ...
	   //*********************** UPDATE MY COLLECTION **********************
        ...
        $query = $em->createQuery("UPDATE BundleAdminBundle:KidsKulaStudentHasCollection e "
                    . "SET e.collectionId = ".$myColNewId->getId() ." , e.tradewith =".$entity->getSenderId()->getId()
                    . " , e.sharedate = '" . date("Y-m-d H:i:s")."'"
                    . " WHERE e.studentId = " .$entity->getReceiverId()->getId() 
                    ." AND e.collectionId = ".$frndColNewId->getId(). " AND e.getcollection = 1" );
        $query->execute();
        ...
        $query = $em->createQuery("UPDATE BundleAdminBundle:KidsKulaStudentHasCollection e "
                    . "SET e.collectionId = ".$frndColNewId->getId() ." , e.tradewith =".$entity->getReceiverId()->getId()
                    . " , e.sharedate = '" . date("Y-m-d H:i:s")."'"
                    . " WHERE e.studentId = " .$entity->getSenderId()->getId() 
                    ." AND e.collectionId = ".$myColNewId->getId(). " AND e.getcollection = 1" );
        $query->execute();
