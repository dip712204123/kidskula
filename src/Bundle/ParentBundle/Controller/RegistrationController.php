...
{			
        ...
        if($request->get('auth'))
		  ...
		  if($form->isValid())  {
                ...
                if (!empty($checkparent)) {
                    $this->get('session')->getFlashBag()->set('error_message', 'This email already registered.');
                    return $this->redirect($this->generateUrl('parent_registration'));
                }
	...
	{	
                $em = $this->getDoctrine()->getManager();
                $parentDetails = $em->getRepository('BundleAdminBundle:KidsKulaUsers')->findOneBy(array('confirmationToken' => $token));
                
                $findallchildofparent = $em->getRepository('BundleAdminBundle:KidsKulaUsers')->findBy(array('email' => $parentDetails->getEmail()));
               // $userarray=array();
              
                foreach($findallchildofparent as $roles)
                {
                   $rolearray=$roles->getRoles();
                  
                  if (in_array('ROLE_STUDENT', $rolearray)) 
                    {
                       $userid= $roles->getId();
                      //$userarray[] = $roles->getId();
                        $userentity = $em->getRepository('BundleAdminBundle:KidsKulaUsers')->findOneBy(array('id' => $userid));                      
                        $userentity->setParentid($parentDetails->getId());
              
                        $em->flush();
                    }
                    //$userid=$roles->getId();
                   // echo '<pre>';
                   // \Doctrine\Common\Util\Debug::dump($userarray);exit;
                }
                
                
		$securityContext = $this->container->get('security.context');
		if ($securityContext->isGranted('IS_AUTHENTICATED_FULLY')) {
			return new RedirectResponse($this->generateUrl('kidskula_bundle_parents'));            
		}
                
                $this->get('session')->getFlashBag()->set('success', 'You are successfully verified your email now you can login again');
                return $this->redirect($this->generateUrl('kidskula_bundle_parents'));
