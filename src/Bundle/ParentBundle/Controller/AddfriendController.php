...
class AddfriendController extends Controller {
    ...
    public function addfriendAction(Request $request) {
        ...
        //$parentApproval= $this->get('Bundle_userservice')->getParentfrndApproval();  //not needed for parents
    ...
    public function invitefrndsAction(Request $request) {
        ...
        $em = $this->getDoctrine()->getManager();
        //check parent approval for friend request from user manager
        //$parentApproval= $this->get('Bundle_userservice')->getParentfrndApproval(); //does not need for parent
        //****************CHECKING FOR ROLE PARENT*****************************
            $parent = $em->createQueryBuilder();
            $parent->select('a')->from('BundleAdminBundle:KidsKulaUsers', 'a')
            
            ->where("a.securityCode ='" . $secretcode."'")
            ->andwhere("a.myavatar='". $avatarname."'")
            ->andwhere("a.roles= 'ROLE_PARENT'");
            
             $query = $parent->getQuery();
             echo $query->getSql();exit;
             $parentcheck = $query->getResult(\Doctrine\ORM\Query::HYDRATE_ARRAY);
              echo '<pre>';
            \Doctrine\Common\Util\Debug::dump($parentcheck);exit;
          // ************************END CHECKING***************  
        if ($type == 'invitewithsecret') {
            ...
            echo '<pre>';
            \Doctrine\Common\Util\Debug::dump($friendId);exit;
            if ($friendId) {
                $userId = $this->getUser()->getid();
