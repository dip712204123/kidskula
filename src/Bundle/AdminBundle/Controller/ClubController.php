...
class ClubController extends Controller {
    ...
    /**
     * Lists all KidsKulaClub entities.
     *
     */
    public function indexAction() {
        ...
        $category->select('a')
                 ->from('BundleAdminBundle:KidsKulaClub', 'a')
				 ->where('a.status <> 3')
                 ->orderBy('a.id', 'DESC');
        ...
        return $this->render('BundleAdminBundle:Club:index.html.twig', array(
                    'entities' => $pagination));
    }
    ...
    public function statusAction($id) {
        ...
        $statement = $connection->prepare("UPDATE kidskula_club SET status = BINARY(status=0) where id='" . $id . "'");      
        $statement->execute();
