...
class ContactusController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        if($request->getMethod()=='POST'){
             ...
             $sort_parameter = $request_data->get('sort_by');
             //print_r($request_data->get('sort_by')); 
             //exit;
             $category->select('a')
                ->from('BundleAdminBundle:Contactus', 'a')
                ->where('a.status <> 3')
                ->andwhere('a.registeredas =:sort_parameter')
                ->orderBy('a.id', 'DESC')
                ->setParameter('sort_parameter', $sort_parameter);
    ...
    public function showAction(Request $request, $id) {
        ...
        return $this->render('BundleAdminBundle:Contactus:show.html.twig', array('entities' => $details));
    }
