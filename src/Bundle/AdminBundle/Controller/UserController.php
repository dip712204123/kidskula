...
class UserController extends Controller {
    ...
    public function loginAction(Request $request) {
        ...
        if($this->container->get('security.context')->isGranted('ROLE_ADMIN')){
             return $this->redirect($this->generateUrl('bundle_admin_homepage'));
        }
        ...
        if ($request->getMethod() == 'POST'):
            ...
            if (empty($username) || empty($password)) {
                ...
                return $this->redirect($this->generateUrl('bundle_admin_logincheck'));
            }
            ...
            if (!$user) {
                ...
                $rolepermitted = array('ROLE_ADMIN');
                $roles = $user->getRoles();
                $diff = array_diff($rolepermitted, $roles);
                ...
                if ($Passwordresult == $user->getPassword()&& count($diff) != 1) {
